const select = document.querySelector("#things");
...
let filterData = [...UzumData];

UzumData.forEach((item) => {
  item.catalogCard.feedbackQuantity;
});

select.addEventListener("change", (e) => {
  if (e.target.value == "arzonroq") {
    const filteredData = UzumData.filter(
      (item) => item.catalogCard.minFullPrice <= 20000000
    );
    result.textContent = "";
  ...
  } else if (e.target.value == "qimmatroq") {
    const filteredData = UzumData.filter(
      (item) => item.catalogCard.minFullPrice > 20000000
    );
    result.textContent = "";
...
function reder() {
  ...
  if (!filterData.length) {
    result.textContent = 'bu filter boyicha ma"lummot topilmadi';
  }
